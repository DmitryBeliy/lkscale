#!/usr/bin/env node
/**
 * Supabase Migration with conflict resolution
 */
...
async function batchInsert(table, rows, attempt = 1) {
  try {
    ...
    let conflictCols = 'id';
    if (table === 'products') conflictCols = 'sku';
    if (table === 'orders') conflictCols = 'order_number';

    const { data, error } = await supabase
      .from(table)
      .upsert(rows, { onConflict: conflictCols, ignoreDuplicates: true })
      .select();
  ...
  for (const insert of inserts) {
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      if (result.success) {
        ...
        console.log(`FAILED - Table ${table} missing`);
