...
# Database connection - using Supabase connection pooler
# Format: postgresql://postgres:[password]@db.[project-ref].supabase.co:5432/postgres
# Or using connection pooler: postgresql://postgres.[project-ref]:[password]@aws-0-[region].pooler.supabase.com:6543/postgres

# For Supabase, we need to construct the connection string
# We'll use the pooler which doesn't require IP whitelisting
PROJECT_REF = "onnncepenxxxfprqaodu"

# Get password from environment or prompt
DB_PASSWORD = os.environ.get('SUPABASE_DB_PASSWORD')

# Connection string using Supabase pooler (transaction mode)
# This uses the service_role key as password for the pooler
DB_HOST = f"db.{PROJECT_REF}.supabase.co"
DB_PORT = 5432
DB_NAME = "postgres"
DB_USER = "postgres"

# Alternative: use direct connection if password is available
# DB_URL = f"postgresql://{DB_USER}:{DB_PASSWORD}@{DB_HOST}:{DB_PORT}/{DB_NAME}"

def get_connection():
    """Get database connection"""
    if not DB_PASSWORD:
        print("❌ Database password not set. Set SUPABASE_DB_PASSWORD environment variable.")
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if attempt < MAX_RETRIES:
                print(f"   ⚠️  Attempt {attempt} failed, retrying in {RETRY_DELAY * attempt}s...")
    ...
    if has_large_inserts(sql):
        ...
        if not batches:
            ...
            for i, batch in enumerate(batches, 1):
                ...
                if success:
                    print(f"\r   ✅ Batch {i}/{len(batches)}")
                else:
                    print(f"\r   ❌ Batch {i}/{len(batches)} failed: {error}")
                    return False
