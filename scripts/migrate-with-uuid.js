#!/usr/bin/env node
/**
 * Supabase Migration with UUID generation
 * Replaces gen_random_uuid() with generated UUIDs
 */
...
function parseInsertStatements(sql) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inInsert && line.startsWith('(')) {
      // Replace gen_random_uuid() with generated UUID
      ...
      columns.forEach((col, idx) => {
        // Skip cost_price for order_items if table doesn't have it
        if (tableName === 'order_items' && col === 'cost_price') {
          return;
        }
...
async function batchInsert(table, rows, attempt = 1) {
  try {
    // For products table, use SKU as conflict column; for others use id
...
async function runMigration() {
  ...
  console.log('Supabase Migration via REST API with UUID Generation');
  console.log('='.repeat(60));
